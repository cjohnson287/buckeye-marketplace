...
**Milestone 3:** Focus on ProductsController with 2 GET endpoints and in-memory data store.
...
**ProductsController** with exactly 2 endpoints:
1. `GET /api/products` — returns all products (HTTP 200)
2. `GET /api/products/{id}` — returns single product (HTTP 200) or 404 if not found

**In-Memory Data Store:**
- Private static list of `Product` objects (at least 8 products)
- Categories: at least 3 (e.g., Textbooks, Electronics, Furniture, Clothing)
- Realistic names, prices ($9.99–$999.99), seller names, and placeholder image URLs

**Product C# Model** (all fields required):
...
- `PostedDate` (DateTime) — when listing was created
...
### M3: ProductsController

For Milestone 3, create a simple ProductsController with 2 GET endpoints using an in-memory static list:
...
{
    // Private static list of products (in-memory data store)
    private static readonly List<Product> Products = new()
    {
        new Product { Id = 1, Title = "Intro to Computer Science", Description = "Used textbook in good condition", Price = 45.99m, Category = "Textbooks", SellerName = "Alice", PostedDate = DateTime.UtcNow.AddDays(-10), ImageUrl = "https://picsum.photos/300/400?random=1" },
        new Product { Id = 2, Title = "Mechanical Keyboard", Description = "RGB backlit, mechanical switches", Price = 89.99m, Category = "Electronics", SellerName = "Bob", PostedDate = DateTime.UtcNow.AddDays(-5), ImageUrl = "https://picsum.photos/300/400?random=2" },
        // ... add 6+ more products across 3+ categories
    };
...
- **No async** — static data doesn't need Task/await in M3.
- **Return the Product object directly** — ControllerBase serializes it to JSON automatically.
- **HTTP 200 for success**, **404 for not found** — no other status codes needed for M3.
- **Minimum 8 products** across at least 3 categories (Textbooks, Electronics, Furniture, Clothing).
- **Realistic data** — use actual prices ($9.99–$999.99), seller names, and descriptions.
- **Placeholder images** — use picsum.photos or similar: `https://picsum.photos/300/400?random={id}`
...
// CORS for React dev server (M3 requirement)
...
- **Allow GET requests** (POST, PUT, DELETE not needed for M3).
- **Enable Swagger** in development (useful for debugging).
- **No middleware beyond basics** — keep it simple in M3.
- **No database setup** — just static data in the controller.
